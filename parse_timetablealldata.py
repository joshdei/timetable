...
def parse_timetable(doc_path):
    ...
    # FIX: iterate over ALL tables in the document, not just tables[0]
    for table in doc.tables:
        for row in table.rows:
            ...
            # Corrected regex — no double-backslash inside raw string
            date_match = re.search(r'(\d{1,2}(?:st|nd|rd|th)?\s+[A-Za-z]+,?\s+\d{4})', ' '.join(cells))
            if date_match:
                current_date = date_match.group(1).strip()

            # Update day_name only when cells[1] is a meaningful non-date string
            ...
            # Drop junk header rows where the code IS the time label
...
def save_sql(data, path, table='timetables'):
    lines = []
    lines.append(f"-- Timetable INSERT statements")
    lines.append(f"-- Generated by parse_timetable.py")
    lines.append(f"-- Total records: {len(data)}\n")
    lines.append(f"INSERT INTO `{table}` (`exam_date`, `type_of_time_table`, `time_slot`, `course_code`, `course_title`, `status`, `created_at`, `updated_at`) VALUES")

    values = []
    for row in data:
        exam_date          = row['exam_date'].replace("'", "''")          if row['exam_date']          else ''
        type_of_time_table = row['type_of_time_table'].replace("'", "''") if row['type_of_time_table'] else ''
        time_slot          = row['time_slot'].replace("'", "''")          if row['time_slot']          else ''
        course_code        = row['course_code'].replace("'", "''")        if row['course_code']        else ''
        course_title       = row['course_title'].replace("'", "''")       if row['course_title']       else ''
        status             = row['status'].replace("'", "''")             if row['status']             else 'active'

        values.append(
            f"  ('{exam_date}', '{type_of_time_table}', '{time_slot}', "
            f"'{course_code}', '{course_title}', '{status}', NOW(), NOW())"
        )
...
def main():
    ...
    sql_path   = r'c:/Users/Devaris/Desktop/timetbale/aligned_timetable.sql'
